package com.sunnao.aibox.module.infra.service.codegen.inner;
...
import com.sunnao.aibox.framework.apilog.core.enums.OperateTypeEnum;
...
import com.sunnao.aibox.framework.common.util.date.DateUtils;
import com.sunnao.aibox.framework.common.util.date.LocalDateTimeUtils;
import com.sunnao.aibox.framework.common.util.object.BeanUtils;
import com.sunnao.aibox.framework.common.util.object.ObjectUtils;
...
import com.sunnao.aibox.module.infra.enums.codegen.CodegenFrontTypeEnum;
import com.sunnao.aibox.module.infra.enums.codegen.CodegenSceneEnum;
import com.sunnao.aibox.module.infra.enums.codegen.CodegenTemplateTypeEnum;
import com.sunnao.aibox.module.infra.enums.codegen.CodegenVOTypeEnum;
import com.sunnao.aibox.module.infra.framework.codegen.config.CodegenProperties;
...
public class CodegenEngine {
    ...
    /**
     * 是否为 aibox-cloud 项目，用于解决 Boot 和 Cloud 的 api 模块兼容性问题
     *
     * true  - 需要有 aibox-module-xxx-api 模块
    ...
    public CodegenEngine() {
        ...
        this.cloudEnable = ClassUtils.isPresent("com.sunnao.aibox.module.infra.framework.rpc.config.RpcConfiguration",
    ...
    private Map<String, String> getTemplates(Integer frontType) {
        ...
        if (Boolean.FALSE.equals(cloudEnable)) {
            SERVER_TEMPLATES.forEach((templatePath, filePath) -> {
                filePath = StrUtil.replace(filePath, "/aibox-module-${table.moduleName}-api", "");
                filePath = StrUtil.replace(filePath, "/aibox-module-${table.moduleName}-server", "");
    ...
    private static String javaModuleFilePath(String path, String module, String src) {
        return "aibox-module-${table.moduleName}/" + // 顶级模块
                "aibox-module-${table.moduleName}-" + module + "/" + // 子模块
                "src/" + src + "/java/${basePackage}/module/${table.moduleName}/" + path + ".java";
    }

    private static String mapperXmlFilePath() {
        return "aibox-module-${table.moduleName}/" + // 顶级模块
                "aibox-module-${table.moduleName}-server/" + // 子模块
    ...
    private static String vueFilePath(String path) {
        return "aibox-ui-${sceneEnum.basePackage}-vue2/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3FilePath(String path) {
        return "aibox-ui-${sceneEnum.basePackage}-vue3/" + // 顶级目录
                "src/" + path;
    }
